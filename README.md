...
At the end of a prediction, YOLO returns a set of bounding boxes with different confidence levels assigned to them. Now the algorithm non-max supression is used to filter out unnecessary or overlapping bounding boxes. To detect overlapping bounding boxes, intersection over union is used. In the end only the most promising bounding boxes (highest confidence and no overlapping) survive the postprocessing done in non-max supression.
